...
##### GoApi
In a more complex scenario when I want to run multiple API endpoint locally to test some microservices I can use <Link href={dockerCompose} target="_blank">docker compose</Link>to define multiple container instances and start them as a group using docker-compose up.  Here I create a docker-compose.yml file that mounts the same source code (volumes) to 3 separate containers (api1, api2, api3) using the same base image (golang:1.16.2-buster) from the Go team and map the container port (1323) to different host ports (3140, 3150, 3160).
